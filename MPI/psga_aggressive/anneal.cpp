...
int** evolveGeneration(int** generation, int*& cost) {
	...
	for (int j = 0; j < size; j++) {
		...
		if (second) {
			swap(new_cost[j],cost[minIdx]);
			swap(new_generation[j],generation[minIdx]);
		} else if (minIdx != j) {
			swap(new_cost[j],new_cost[minIdx]);
			swap(new_generation[j],new_generation[minIdx]);
		}
	}
